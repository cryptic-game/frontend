...
describe('GlobalCursorService', () => {
  ...
  it('#requestCursor() should set the cursor of the document body and return a number if the current lock is undefined', inject(
    [GlobalCursorService],
    (service: GlobalCursorService) => {
  ...
  it('#requestCursor() should return -1 and should not set the cursor if the current lock is defined', inject(
    [GlobalCursorService],
    (service: GlobalCursorService) => {
  ...
  it('#requestCursor() should overwrite the current cursor if the lock parameter is valid', inject(
    [GlobalCursorService],
    (service: GlobalCursorService) => {
  ...
  it('#releaseCursor() should reset the cursor of the document body, reset the lock and return true if the lock is valid', inject(
    [GlobalCursorService],
    (service: GlobalCursorService) => {
      const lock = 15;
  ...
  it('#releaseCursor() should not the cursor of the document body and return false if the lock is not valid', inject(
    [GlobalCursorService],
    (service: GlobalCursorService) => {
