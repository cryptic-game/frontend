...
describe('HardwareService', () => {
  let webSocket: any; //TODO: Type me correct

  beforeEach(() => {
    webSocket = webSocketMock();

    TestBed.configureTestingModule({
      providers: [{ provide: WebsocketService, useValue: webSocket }, HardwareService],
    });
  });
  ...
  it('#updateParts() should call /hardware/list and save the hardware parts in a variable if there was no error', inject(
    [HardwareService],
  ...
  it('#updateParts() should call /hardware/list and do nothing if there was an error', inject(
    [HardwareService],
    (service: HardwareService) => {
      service.hardwareAvailable = null;
  ...
  it('#getAvailableParts() should return the saved hardware parts', inject(
    [HardwareService],
    (service: HardwareService) => {
      spyOn(service, 'updateParts').and.returnValue(
  ...
  it('#getAvailableParts() should call updateParts() if there are no saved hardware parts yet', inject(
    [HardwareService],
    (service: HardwareService) => {
      const hardware = new HardwareList();
      hardware.ram['test'] = {
  ...
  it('#getDeviceParts() should call /device/info and return the corresponding hardware parts from hardwareAvailable', inject(
    [HardwareService],
    (service: HardwareService) => {
