...
export class FileService {
  ...
  getFiles(deviceUUID: string, parentUUID: string = Path.ROOT!): Observable<File[]> {
    return this.webSocket.ms('device', ['file', 'all'], { device_uuid: deviceUUID, parent_dir_uuid: parentUUID }).pipe(
      map((response: any) => {
  ...
  getFilesRecursively(deviceUUID: string, parentUUID: string = Path.ROOT!): Observable<File[]> {
    return this.getFiles(deviceUUID, parentUUID).pipe(
      mergeMap((files) => {
        const allChildren = files
          .filter((f) => f.is_directory)
          .map((f) => this.getFilesRecursively(deviceUUID, f.uuid).pipe(take(1)));
        if (allChildren.length === 0) {
          return of(files);
        } else {
          return combineLatest(allChildren).pipe(
            map((children) => {
  ...
  getAbsolutePath(deviceUUID: string, fileId: string | null): Observable<string[]> {
    ...
    const getTree: (file: File) => Observable<string[]> = (file: File) => {
      ...
      return this.getFile(deviceUUID, file.parent_dir_uuid).pipe(
        mergeMap((parent) => {
          return getTree(parent).pipe(
            map((tree) => {
    ...
    return this.getFile(deviceUUID, fileId).pipe(
      mergeMap((file) => {
        return getTree(file);
      })
    );
  ...
  getFromPath(deviceUUID: string, path: Path): Observable<File> {
    ...
    if (path.path[0] === '..') {
      return this.getFile(deviceUUID, path.parentUUID).pipe(
        mergeMap((par) => {
          if (path.path.length === 1) {
    ...
    return this.getFiles(deviceUUID, path.parentUUID).pipe(
      mergeMap((files) => {
  ...
  moveToPath(source: File, destPath: Path): Observable<File> {
    ...
    return this.getFromPath(deviceUUID, destPath).pipe(
      mergeMap((destination) => {
        if (!destination.is_directory) {
          throw new Error('destination_is_file');
        }
        return this.getFiles(deviceUUID, destination.uuid).pipe(
          mergeMap((destinationFiles) => {
      ...
      catchError((err) => {
        if (err.message === 'file_not_found') {
          if (destPath.path.length <= 1) {
            ...
            return this.getFromPath(deviceUUID, destDirPath).pipe(
              catchError((error) => {
