...
@Component({
  ...
  animations: [
    trigger('powerButton', [
      transition('* <=> *', [query('@powerButtonFill, @powerButtonStroke, @powerButtonProgress', [animateChild()])]),
    ]),
    ...
    trigger('powerButtonProgress', [
      ...
      transition('* => on', [animate('20s')]),
      transition('* => off', [animate('30s')]),
    ]),
  ],
...
export class ControlCenterDevicePageComponent implements OnDestroy {
  ...
  updateServices(): void {
    ...
    if (this.device.powered_on) {
      this.deviceService.getDeviceResourceUsage(this.device.uuid).subscribe((resources) => {
        ...
        this.webSocket
          .ms('service', ['list'], { device_uuid: this.device.uuid })
          .pipe(
            switchMap((response) => from(response.services as { uuid: string; name: string; running: boolean }[])),
            filter((service) => service.running),
            mergeMap((service) =>
              this.deviceService.getServiceResourceUsage(service.uuid).pipe(
                map((serviceUsage) => ({
                  service: service,
                  usage: new ResourceUsage(serviceUsage).relativeToDevice(this.deviceResources),
                }))
              )
            ),
            toArray(),
  ...
  finishRenaming(): void {
    ...
    const newName = this.deviceRenameField.nativeElement.innerText.replace(/[^a-zA-Z0-9\-_]+/g, '').substring(0, 15);
    if (newName.length === 0) {
      this.stopRenaming();
      return;
    }
  ...
  powerButtonClicked() {
    if (!this.powerButton.animating) {
      ...
      this.powerButton.state = this.powerButton.state === 'fast-off' || this.powerButton.state === 'off' ? 'on' : 'off';
    }
  }
