...
export class HardwareShopHeaderComponent implements OnDestroy {
  ...
  constructor(private walletAppService: WalletAppService, private hardwareShopService: HardwareShopService) {
    this.subscriptions.add(this.walletAppService.update.subscribe((wallet) => (this.morphCoins = wallet?.amount!)));
    this.subscriptions.add(this.hardwareShopService.updateCartItems.subscribe(() => this.updateCartSize()));
