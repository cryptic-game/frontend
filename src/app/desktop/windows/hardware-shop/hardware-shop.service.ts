...
export class HardwareShopService {
  ...
  constructor(
    ...
    hardwareService
      .getAvailableParts()
      .pipe(switchMap(() => this.updateHardwareParts()))
      .subscribe(() => {
  ...
  async loadCartItems(): Promise<void> {
    ...
    this.cartItems = items
      .filter((item) => storedCart.hasOwnProperty(item.part.id))
      .map((shopItem) => ({
        id: shopItem.part.id,
  ...
  setCartItems(items: HardwareShopCartItem[], save = true): void {
    ...
    if (save) {
      this.settingService.set(
        'shop_cart',
        JSON.stringify(items.reduce((acc, item) => ({ ...acc, [item.id]: item.quantity }), {}))
      );
    }
  ...
  updateHardwareParts(): Observable<void> {
    return this.websocketService.ms('inventory', ['shop', 'list'], {}).pipe(
      map((data) => {
