...
export class FileManagerComponent extends WindowComponent implements OnInit, OnDestroy {
  ...
  navigateByAddress() {
    this.fileService.getFromPath(this.delegate.device.uuid, Path.fromString(this.addressBarURL, Path.ROOT!)).subscribe({
      next: (result: File) => {
  ...
  dragOver(event: DragEvent, destination: File) {
    if (
      ...
      event.dataTransfer.types[0] === 'cryptic/file'
    ) {
  ...
  dropCopy(event: { event: Event; item: { file: File; destinationUUID: string } }) {
    ...
    this.fileService.createFile(this.delegate.device.uuid, file.filename, file.content, destinationUUID).subscribe({
      error: (e: Error) => {
        // @ts-ignore
        if (e.message === 'file_already_exists') {
          this.displayError('A file with the same name already exists.');
  ...
  newFile(name: string) {
    this.fileService.createFile(this.delegate.device.uuid, name, '', this.currentFolder.uuid).subscribe({
      error: (e: Error) => {
        // @ts-ignore
        if (e.message === 'file_already_exists') {
          this.displayError('A file with the same name already exists.');
  ...
  async delete(file: File) {
    ...
    if (file.is_directory) {
      confirmed = await this.displayConfirm(
        'Are you sure you want to delete this folder? You might lose access to a wallet.'
      );
