...
export class DefaultTerminalState extends CommandTerminalState {
  ...
  constructor(
    ...
    protected activeDevice: Device,
    protected terminal: TerminalAPI,
    public promptColor: string | null = null
  ) {
    super();
  ...
  static promptAppender(value: string): string {
    return `<span class="promptAppender" style="text-decoration: underline; cursor: pointer;">${escapeHtml(
      value
    )}</span>`;
  }
  ...
  refreshPrompt() {
    this.fileService.getAbsolutePath(this.activeDevice['uuid'], this.working_dir!).subscribe((path) => {
      const color = this.domSanitizer.sanitize(
  ...
  miner(args: string[]) {
    ...
    if (args[0] === 'look') {
      this.websocket
        ...
        .subscribe((listData) => {
          listData.services.forEach((service: any) => {
            //TODO: Type me correct
            if (service.name === 'miner') {
              miner = service;
              this.websocket
                .ms('service', ['miner', 'get'], {
                ...
                .subscribe((data) => {
                  ...
                  text =
                    ...
                    '<br>' +
                    'Mining Speed: ' +
                    String(formatNumber(Number(miner.speed) * 60 * 60, this.locale)) +
                    ' MC/h<br>' +
                    'Power: ' +
                    power +
                    '%';
  ...
  hostname(args: string[]) {
    if (args.length === 1) {
      ...
      this.websocket.ms('device', ['device', 'info'], { device_uuid: this.activeDevice['uuid'] }).subscribe({
        next: (device: Device) => {
  ...
  list_files(files: File[]) {
    files
      ...
      .sort()
      .forEach((folder) => {
        forkJoin([this.settings.terminalLsFolderColor.get(), this.settings.terminalLsPrefix.get()]).subscribe(
          ([folderColor, lsPrefix]) => {
            this.terminal.output(
              `<span style="color: ${folderColor};">${lsPrefix ? '[Folder] ' : ''}${folder.filename}</span>`
            );
          }
        );
      });
    ...
    files
      ...
      .sort()
      .forEach((file) => {
        this.settings.terminalLsPrefix
          .get()
          .subscribe((lsPrefix) => this.terminal.outputText(`${lsPrefix ? '[File] ' : ''}${file.filename}`));
      });
  }
      ...
      this.fileService.getFromPath(this.activeDevice['uuid'], path).subscribe({
        next: (target) => {
          if (target.is_directory) {
            this.fileService
              .getFiles(this.activeDevice['uuid'], target.uuid)
              .subscribe((files) => this.list_files(files));
          } else {
            this.terminal.outputText('That is not a directory');
          }
        },
                      ...
                      (answer) => {
                        if (answer) {
                          this.websocket.ms('currency', ['delete'], { source_uuid: uuid, key: key }).subscribe({
                            ...
                            error: (error: Error) => {
                              this.terminal.output(
                                '<span class="error-text"">The wallet couldn\'t be deleted successfully. ' +
                            ...
                            .pipe(
                              catchError((err: Error) => {
                                this.terminal.outputText(
                                  "That file couldn't be created. Please note your wallet credentials " +
                                    "and put them in a new file with 'touch' or contact the support: '" +
                                    credentials +
                                    "'"
                                );
                                reportError(err);
  ...
  pay(args: string[]) {
    if (args.length === 3 || args.length === 4) {
      ...
      if (isNaN(parseFloat(amount)) || parseFloat(amount) <= 0) {
        ...
        this.fileService.getFromPath(this.activeDevice['uuid'], walletPath).subscribe(
          (walletFile) => {
  ...
  service(args: string[]) {
    ...
    const getServices = () =>
      this.websocket.ms('service', ['list'], { device_uuid: activeDevice }).pipe(
        map((data) => {
    ...
    if (args.length >= 1 && args[0].toLowerCase() === 'create') {
      ...
      getService('bruteforce').subscribe((bruteforceService) => {
        ...
        const startAttack = () => {
          this.websocket
            .ms('service', ['bruteforce', 'attack'], {
              service_uuid: bruteforceService['uuid'],
              device_uuid: activeDevice,
            ...
            .subscribe({
              next: () => {
                ...
                this.terminal.pushState(
                  new BruteforceTerminalState(this.terminal, this.domSanitizer, (stop) => {
                    if (stop) {
          ...
          .subscribe({
            next: (status) => {
              ...
              if (differentServiceAttacked) {
                ...
                div.innerHTML =
                  'The bruteforce service already attacks another device: ' +
      ...
      getService('portscan').subscribe((portscanService) => {
        ...
        this.websocket
          .ms('service', ['use'], {
            service_uuid: portscanService['uuid'],
            device_uuid: activeDevice,
            target_device: targetDevice,
          })
          .subscribe((data) => {
    ...
    this.websocket.ms('device', ['device', 'spot'], {}).subscribe((random_device) => {
      this.websocket.ms('service', ['list'], { device_uuid: this.activeDevice['uuid'] }).subscribe((localServices) => {
        ...
        this.websocket
          .ms('service', ['use'], {
            device_uuid: this.activeDevice['uuid'],
            service_uuid: portScanner['uuid'],
            target_device: random_device['uuid'],
          })
          .subscribe({
            next: (remoteServices) => {
              ...
              list.innerHTML =
                '<li>UUID: ' +
                DefaultTerminalState.promptAppender(random_device['uuid']) +
                '</li>' +
                '<li>Services:</li>' +
                '<ul>' +
      ...
      if (args[0] === 'public') {
          ...
          if (networks != null && networks.length !== 0) {
            ...
            networks.forEach((network: any) => {
              ...
              element.innerHTML +=
                ...
                DefaultTerminalState.promptAppender(network['uuid']) +
                '</span>';
            });
        ...
        this.websocket.ms('network', ['member'], data).subscribe((memberData) => {
          ...
          if (memberNetworks != null && memberNetworks.length > 0) {
            ...
            memberNetworks.forEach((network: any) => {
              //TODO: Type me correct
              ...
              invitations.forEach((invitation: any) => {
                ...
                this.websocket.ms('network', ['get'], { uuid: invitation['network'] }).subscribe((network) => {
                  element.innerHTML +=
                    '<br>Invitation: ' +
                    '<span style="color: grey">' +
                    DefaultTerminalState.promptAppender(invitation['uuid']) +
                    '</span><br>' +
                    'Network: ' +
                    escapeHtml(network['name']) +
                    '<br>' +
                    'Owner: ' +
                    '<span style="color: grey">' +
                    DefaultTerminalState.promptAppender(network['owner']) +
                    '</span><br>';
                  DefaultTerminalState.registerPromptAppenders(element);
                });
              });
      ...
      if (args[0] === 'delete') {
          ...
          next: (requestsData) => {
            ...
            if (requests.length === 0) {
              ...
              requests.forEach((request: any) => {
                //TODO: Type me correct
                element.innerHTML +=
                  '<br>Request: <span style="color: grey;">' +
                  DefaultTerminalState.promptAppender(request['uuid']) +
                  '</span><br>' +
                  'Device: <span style="color: grey;">' +
                  DefaultTerminalState.promptAppender(request['device']) +
                  '</span><br>';
              });
        ...
        this.websocket.ms('network', [args[0] === 'info' ? 'get' : 'name'], data).subscribe({
          next: (getData) => {
            const element = document.createElement('div');
            element.innerHTML =
              'UUID: <span style="color: grey;">' +
              DefaultTerminalState.promptAppender(getData['uuid']) +
              '</span><br>';
            element.innerHTML += 'Name: ' + escapeHtml(getData['name']) + '<br>';
            element.innerHTML += 'Hidden: ' + (getData['hidden'] ? 'private' : 'public') + '<br>';
            element.innerHTML +=
              'Owner: <span style="color: grey;">' + DefaultTerminalState.promptAppender(getData['owner']) + '</span>';
          ...
          next: (membersData) => {
            ...
            if (members != null && members.length > 0) {
              ...
              members.forEach((member: any) => {
                //TODO: Type me correct
                this.websocket
                  .ms('device', ['device', 'info'], { device_uuid: member['device'] })
                  .subscribe((deviceData) => {
                    element.innerHTML +=
                      ' <span style="color: grey">' +
                      DefaultTerminalState.promptAppender(member['device']) +
                      '</span> ' +
                      deviceData['name'] +
                      '<br>';
                  });
  ...
  info() {
    ...
    const element = document.createElement('div');
    element.innerHTML = `Address: <span style="color: silver;">${DefaultTerminalState.promptAppender(
      this.activeDevice['uuid']
    )}</span>`;
    this.terminal.outputNode(element);
