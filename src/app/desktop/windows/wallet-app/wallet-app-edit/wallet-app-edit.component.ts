...
export class WalletAppEditComponent implements OnInit {
  form = new FormGroup({
    uuid: new FormControl('', [Validators.required, Validators.pattern(WalletAppService.WALLET_UUID_REGEX)]),
    key: new FormControl('', [Validators.required, Validators.pattern(WalletAppService.WALLET_KEY_REGEX)]),
  });
