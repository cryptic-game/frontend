...
export class MinerComponent extends WindowComponent implements OnInit {
  ...
  walletControl: FormControl = new FormControl('', [
    Validators.required,
    Validators.pattern(/^[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$/),
  ]);
  wallet: string | undefined;
  errorMessage: string;

  minerPower: FormControl = new FormControl(0, [Validators.required, Validators.min(0), Validators.max(100)]);
