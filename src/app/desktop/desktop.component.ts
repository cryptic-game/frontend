...
export class DesktopComponent implements OnInit {
  ...
  mouseMove(e: MouseEvent) {
    if (this.dragLinkageIndex !== undefined) {
      ...
      this.dragElement.style.left =
        Math.min(
          Math.max(e.pageX - surfaceBounds.left - this.dragOffset.x, 0),
          surfaceBounds.width - this.dragElement.clientWidth
        ) + 'px';
      this.dragElement.style.top =
        Math.min(
          Math.max(e.pageY - surfaceBounds.top - this.dragOffset.y, 0),
          surfaceBounds.height - this.dragElement.clientHeight - taskBarHeight
        ) + 'px';
      if (!this.checkDropAllowed(e)) {
  ...
  checkDropAllowed(e: MouseEvent): boolean {
    ...
    return (
      mouseHoverElement === this.surface.nativeElement ||
      (originalLinkage !== null && mouseHoverElement === originalLinkage)
    );
  }
